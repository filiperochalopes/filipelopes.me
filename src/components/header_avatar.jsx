...
class AvatarHeader extends Component{
  constructor(props){
    ...
    this.state = {
      ...
      toggle3D: () => this.state.vision ? this.setState({ vision: false}) : this.setState({ vision: true})
    }
  }
    ...
    return(
      <div className="wrap">
        <div className="circle">          
            <div id="canvas_3d_avatar" style={{ display: this.state.vision ? "block" : "none"}}></div>
            <div id="canvas_avatar" style={{ display: this.state.vision ? "none" : "block"}}><img src="/img/profile.jpg" /></div>
            <IconButton className="toggle_3d_vision" onClick={this.state.toggle3D}><i className="fas fa-cube"></i></IconButton>
        </div>
      </div>
    );
  }
