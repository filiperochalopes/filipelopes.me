...
import PortfolioSearch from '../components/portfolio_search';
import PortfolioItem from '../components/portfolio_item';
import PortfolioCoverItem from '../components/portfolio_cover_item';

import OnVisible from 'react-on-visible';

import HeaderSlim from '../components/header_slim';
...
class Portfolio extends Component{
  ...
  getPortfolioItem = slug => {
    fetch("http://192.168.0.64/sites/server.filipelopes.me/get_portfolio_item.php?slug="+slug)
    // fetch("https://server.filipelopes.me/get_put_views.php")
    .then(function(response) {
      return response.text();
    }).then((atual) => {     
  ...
  shouldComponentUpdate = (nextProps, nextState) => {
    console.log("chamou should", nextProps.match.params.name == this.props.match.params.name);
  ...
  backgroundArrayPush = (palette, i) => {
    ...
    this.backgroundRef.current.style.backgroundColor = this.portfolioItemsBaseColors[this.state.currentIndex]
  }

  afterSlide = (currentIndex) => {
    if(this.props.match.params.name){ //estava dando erro caso passasse o slide e fechasse antes de completar
      this.setState({
        currentIndex
      }, () => {
        this.backgroundRef.current.style.backgroundColor = this.portfolioItemsBaseColors[this.state.currentIndex]
  ...
  changeIndex = (move) => {
    if(this.backgroundRef.current){
      ...
      if(move == "prev"){
        ...
        this.backgroundRef.current.style.backgroundColor = this.portfolioItemsBaseColors[prev]
      }else if(move == "next"){
        this.setState({
          currentIndex : next
        });
        this.backgroundRef.current.style.backgroundColor = this.portfolioItemsBaseColors[next]
      }
    }
  ...
  render(){
    ...
    if(this.props.match.params.name){
      return (
        <section id="portfolio_item" ref={this.backgroundRef} style={{
          backgroundColor : this.state.backgroundColor }} >
          {this.state.atual.items.map((item, i) => (
            <Palette key={i} image={this.getUrl(item)}>
              {palette => (
                <div className="getBackground" style={{
                  backgroundColor: palette.lightMuted,
                  display: "none"
                }} >{this.backgroundArrayPush(palette, i)}</div>
              )}
            </Palette>
          ))}          
          <header>
          ...
          <div id="carousel" times={this.state.nextTimes}>
            <Carousel
              renderCenterLeftControls={({ previousSlide }) => (
                ...
                }}><i className="fas fa-arrow-left"></i></IconButton>
              )}
              renderCenterRightControls={ ({ nextSlide }) => (
                <IconButton onClick={ () => {
                  nextSlide();
                  this.changeIndex("next");
                }}><i className="fas fa-arrow-right"></i></IconButton>
              )}
            ...
            >
              {this.state.atual.items.map((item, i) => (
                ...
                item={item} key={i} backgroundColor={i} />
              ))}
            </Carousel>
          </div>
          {/* </Palette> */}
          ...
          context => (
            <section>
              <div className="container">
                <h1><Link to="/portfolio">Portfólio <i className="fas fa-external-link-alt"></i></Link> <ShareBt whatsapp="whatsapp://send?text=Veja o meu currículo: https://filipelopes.me/curriculo"/></h1>
                {/* <PortfolioSearch/> */}
                <OnVisible
                  className="flex"
    ...
    return (
      <section ref={this.backgroundRef} style={{
        height: "100vh" }} >
        <HeaderSlim/>
